...
$(document).on('submit', '#form-item', function(event) {
	...
	var iPricePurchase = $('#item-price-purchase').val();
...
function editModal(item_id){
	...
	$.ajax({
			...
			success: function (data) {
				...
				$('#item-price-purchase').val(data.pricepurchase);
				$('#item-id').val(data.id);
				//$('#code').val(data.code);
...
//    );
	...
	$("#search-box").keyup(function(){
		$.ajax({
		type: "POST",
		url: "data/all_item_autocomplete.php",
...
function selectCountry(idProduct,val) {
	$("#search-box").val(val);
	$("#item-id").val(idProduct);
...
function showAllUtility(){
	var date = $('#utiDate').val();
	dailyUtility(date);
}//end showAllSales
showAllUtility();

function dailyUtility(date){
	$.ajax({
			url: 'data/all_utility.php?date='+date,
			type: 'get',
			data: {
				date:date
			},
			success: function (data) {
				$('#all-utility').html(data);
			},
			error:function(){
				eMsg(474);
			}
...
$(document).on('change', '#utiDate', function(event) {
	event.preventDefault();
	/* Act on the event */
	var date = $('#utiDate').val();
	if(date == '' || date == null){
...
$('#utiprint').click(function(event) {
	/* Act on the event */
	var date = $('#utiDate').val();
	window.open('data/print-utility.php?date='+date,'name','width=600,height=400');	
});
